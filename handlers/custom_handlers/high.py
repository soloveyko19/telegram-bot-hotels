""" Модуль отвечающий за команду /high """


from telebot.types import Message, CallbackQuery
from keyboards.inline.keyboard_by_dict import keyboard1
from loader import bot
from states.states import HighStates, AllStates
...
def high_start(message: Message) -> None:
    """
    Функция начала опроса по комманде /high
    ...
    bot.send_message(message.chat.id,
                     'Отлично, будем искать самые дорогие отели!\n'
...
def get_city_info(message: Message) -> None:
    ...
    if len(cities) > 1:
        ...
        bot.set_state(message.from_user.id, HighStates.hotel, message.chat.id)
    else:
        bot.send_message(message.chat.id,
...
@bot.message_handler(state=HighStates.hotel)
    ...
    try:
        ...
        if amt > 10:
            ...
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                ...
                result = get_hotels(data['city_id'], data['hotels_amt'], "PRICE_HIGH_TO_LOW")
                if result:
...
@bot.callback_query_handler(func=lambda call: True, state=HighStates.city)
def city_id_chose(call: CallbackQuery) -> None:
    """
    Функция которая принимает все Callback если пользовательь находится в состоянии HighStates.city
    ...
    bot.send_message(call.message.chat.id, 'Хорошо, давай дальше\n'
                                           'Введи количество отелей для вывода\n'
                                           '(от 1 до 10 включительно)')
    bot.set_state(call.from_user.id, HighStates.hotel, call.message.chat.id)


@bot.callback_query_handler(func=lambda call: True, state=HighStates.hotel)
def hotel_id_chose(call: CallbackQuery) -> None:
    """
    Функция которая принимает все Callback данные за если пользователь находится в состоянии HighStates.hotel
